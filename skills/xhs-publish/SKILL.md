...
description: |
  ...
  完整口播视频链路中作为终点，仅接收通过 review-xhs-video-with-opus 的母版并执行预发布与最终确认。
metadata:
  version: "1.3.0"
  openclaw:
    requires:
...
本 Skill 是链路终点，不再交接给下一个制作 Skill。对于来自这条完整链路的口播视频，上传或预填前必须核验上游交接包：

- `review-xhs-video-with-opus` 的完整评审报告路径；
- Opus 总分不低于 8.0、结论为 `PASS`，且无 P0/P1 阻塞项；
- 复审后最终 MP4、封面和本地 QA 报告的精确路径；
- 最终标题、正文、话题和用户已确认版本；
- 如发生修改，交接的是重渲染并重新 QA 的版本，而不是复审前旧母版。

缺少任一项时，不得把完整链路视频上传或报告为“可发布”。必须返回上一步并明确提示：`使用 review-xhs-video-with-opus 完成复审`。该前置门槛仅适用于本完整口播视频链路，不阻断用户独立提供的普通图文、长文或其他已完成视频。

上游复审通过后，仍须区分两个授权：

1. “预发布”只允许上传、填写标题正文、关联系统推荐话题、设置封面和可见范围，并停在编辑页。
2. 点击“发布”或“定时发布”必须获得用户在预览后的新一轮明确确认；完整链路授权不能替代这次最终确认。
...
- **完整链路必须有 Opus 复审凭证**：来自完整口播视频链路的母版只有在 `review-xhs-video-with-opus` 通过并完成本地复核后，才能进入预发布。
